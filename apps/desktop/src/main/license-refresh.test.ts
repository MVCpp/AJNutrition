...
const REQUEST = { licenseId: 'lic_0001', deviceId: 'dev-1', appVersion: '0.1.0' };
...
describe('licence refresh — what leaves the machine', () => {
  it('sends the licence id, device id and app version, and nothing else', async () => {
    const fetchImpl = respond({});
    ...
    // leaving her machine.
    expect(Object.keys(sent).sort()).toEqual(['appVersion', 'deviceId', 'licenseId']);
    expect(sent).toEqual({ licenseId: 'lic_0001', deviceId: 'dev-1', appVersion: '0.1.0' });
  });
