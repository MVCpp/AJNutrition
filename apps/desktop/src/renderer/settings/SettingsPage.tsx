...
export function SettingsPage() {
  ...
  // Read-only report; loaded on demand because it scans every session.
  const [showDrift, setShowDrift] = useState(false);
  const driftQuery = useQuery({
    queryKey: ['formula-drift'],
    queryFn: () => unwrap(window.ajnutrition.measurement.formulaDrift()),
    enabled: showDrift,
  });
  ...
  return (
    <section aria-labelledby="settings-heading">
      ...
      <div className="mt-6 rounded-xl border border-slate-200 bg-white p-6">
        <h3 className="text-base font-semibold text-slate-800">🧮 {t('settings.formulasTitle')}</h3>
        <p className="mt-1 text-sm text-slate-600">{t('settings.formulasHint')}</p>
        <button
          type="button"
          onClick={() => setShowDrift(true)}
          disabled={driftQuery.isFetching}
          className="mt-3 rounded-md border border-slate-300 px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 disabled:opacity-50"
        >
          {driftQuery.isFetching ? t('settings.formulasChecking') : t('settings.formulasCheck')}
        </button>
        {showDrift && driftQuery.data && (
          <div className="mt-3">
            {driftQuery.data.length === 0 ? (
              <p className="text-sm text-emerald-800">{t('settings.formulasNoDrift')}</p>
            ) : (
              <>
                <p className="mb-2 text-sm text-amber-800">
                  {t('settings.formulasDrift', { count: driftQuery.data.length })}
                </p>
                <ul className="max-h-64 space-y-1 overflow-y-auto text-xs">
                  {driftQuery.data.map((row) => (
                    <li
                      key={`${row.sessionId}-${row.formulaId}`}
                      className="rounded-md bg-amber-50 px-2 py-1 text-amber-900"
                    >
                      Exp. {row.patientFileNumber} · {row.measuredAt} · {row.formulaName}: v
                      {row.storedVersion} = {row.storedResult} {row.unit} → v{row.currentVersion} ={' '}
                      {row.currentResult} {row.unit}
                    </li>
                  ))}
                </ul>
              </>
